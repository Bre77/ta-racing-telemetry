...
def run_script(): 
    ...
    whitelist = {}
    for label in labels:
        arr = map(lambda metric: bool(re.search(opt_whitelist,metric)),labels[label])
        count = sum(arr)
        if count>0:
            whitelist[label] = arr
    ...
    while True:
        try:
            ...
            now = time.time()
            host = addr[0]
            data = dimensions.get(host,{})

            #Check Rate Limiter per IP Address
            ...
            #        values = struct.unpack('<2I 4B bbBBbbb',packet)
                ...
                dimensions[host] = {"car.name": "{}".format(strings[0]),"car.class": "{}".format(strings[1]),"track.name": "{}".format(strings[2]),"track.varient": "{}".format(strings[3])}
